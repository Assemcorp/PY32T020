#include "drivers.h"
#if APP_SMG_ENABLE
extern uint8_t smg_data[COM_COUNT];
/**
 * @brief  7-segment display segment code table
 */
const uint8_t num_tbl[] = {
    0X3F, 0X06, 0X5B, 0X4F, 0X66, 0X6D, 0X7D, 0X07, 0X7F, 0X6F,
};
/**
 * @brief  TK touch keys corresponding to LEDs
 */
const uint8_t TK_LED[] = {
    (1 << 3), (1 << 2), (1 << 1), (1 << 6), (1 << 4), (1 << 5), (1 << 0), (1 << 7)
};
/**
 * @brief  Set default display content on power up
 * @param  None
 * @retval None
 */
void SMG_Default(void)
{
    /* Default display content */
	smg_data[0] = 0X40;
	smg_data[1] = 0X40;
	smg_data[2] = 0X00;
}
/**
 * @brief  Display a decimal number on the 7-segment display
 * @param  num: Number to display
 * @retval None
 */
void SMG_Show(uint16_t num)
{
    smg_data[0] = num_tbl[num / 10];
    smg_data[1] = num_tbl[num % 10];
}
/**
 * @brief  Show LED indicators for touch keys
 * @param  led_bit: Bitmask of LEDs to display (bit0 corresponds to TK0)
 * @retval None
 */
void LED_Show(uint16_t led_bit)
{
    uint8_t i;
    uint8_t bit = 0;
    for (i = 0; i < SEG_COUNT; i++)
    {
        if (led_bit & 0X01)
            bit |= TK_LED[i];
        led_bit >>= 1;
    }
    smg_data[2] = bit;
}

#endif
